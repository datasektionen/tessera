...
export const BusinessDetailsFormSchema = Yup.object().shape({
    ...
    phone_number: Yup.string().required("Required")
        .test('is-valid-phone', 'Invalid phone number', function (value) {
            const phone_code = AvaialableCountries.find((c: Country) => c.code === this.parent.country_code)?.phone;
            try {
                const phoneNumber = phoneUtil.parseAndKeepRawInput(`+${phone_code}${value}`);
